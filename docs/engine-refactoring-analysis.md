# `src/brix/engine.py` Refactoring Analysis

File analyzed: [src/brix/engine.py](/root/docker/brix/src/brix/engine.py)  
Analyzed line count: `2697`

This document is intentionally exhaustive. It describes the full control flow, symbol inventory, path parity gaps, state sharing, dependency surface, test coverage, refactoring risks, and a concrete migration plan for splitting `engine.py` without changing behavior.
...
- `LEGACY_ALIASES` (`101-143`)
  ...
  Public/internal: Public in practice.
...
Sequential execution lives inside [src/brix/engine.py](/root/docker/brix/src/brix/engine.py#L798) through [src/brix/engine.py](/root/docker/brix/src/brix/engine.py#L1431) when `_detect_dag_mode()` returns `False`.
...
DAG execution lives in [src/brix/engine.py](/root/docker/brix/src/brix/engine.py#L2099) through [src/brix/engine.py](/root/docker/brix/src/brix/engine.py#L2528).
...
| Task dispatch / gather | `MISSING` | `2524-2528` | MISSING | Sequential path has direct loop; DAG creates one task per step and gathers with `return_exceptions=True`. |

Major parity conclusion: the DAG path is not just a scheduling variant. It omits validation, foreach, cancellation polling, context snapshotting, progress DB persistence, resource measurement, step-output persistence, step-execution persistence, LLM cost tracking, and saga compensation.
...
- `history`
  Run-history façade used for `record_start`, idempotency lookup, `record_finish`, and cancellation.

- `context`
  Central mutable execution state: inputs, outputs, credentials, workdir, metadata, resume checkpoints, progress snapshots.

- `step_statuses`
  Shared dict of `step_id -> StepStatus`. Read by `else_of`, final result computation, history persistence, alerts, and trigger completion recording.

- `last_output`
  Shared “latest successful output” cursor, used for implicit pipeline result when no explicit `pipeline.output`.

- `pipeline_aborted`
  Shared loop control flag. Set by cancel, stop step, DAG errors, compositor blocks, dependency install failures, execution failures, and outer exceptions.

- `stop_step_success`
  Shared override that controls final `all_ok` if a `stop` step intentionally ended the run.

- `total_cost_usd`
  Shared accumulator updated from cache hits and step successes. Used only in sequential final history persistence.

- `_saga_tracker`
  Shared resilience tracker that records compensatable steps and runs compensations on stop-on-error.

- `jinja_ctx`
  Recomputed frequently; shared only within one iteration, but critical because many branch decisions depend on its freshness after context mutation.

- `rendered_params`, `rendered_step`, `runner`, `result`
  Per-step temporaries passed through multiple later blocks: caching, persistence, status construction, and telemetry.

### DAG shared locals

- `done_events`
  `dict[str, asyncio.Event]`; this is the DAG scheduler’s synchronization fabric.

- `step_ok`
  `dict[str, bool]`; defines whether downstream dependent steps are allowed to run.

- `pipeline_aborted`, `last_output`, `stop_step_success`
  Shared through `nonlocal` closure capture inside `run_step`.

- `step_statuses`
  Shared mutable status map, same conceptual role as in sequential path.
...
- `context.input`
- `context.credentials`
- `context._jinja_cache`
- `context.to_jinja_context(...)`
- `context.is_step_completed(step_id)`
- `context.get_output(step_id)`
- `context.set_output(step_id, value)`
- `context.save_run_metadata(name, status, progress=...)`
- `context.cleanup(keep=...)`
- `context.workdir`
- `context.run_id`
- `context._resume_from`
- `context.load_foreach_checkpoint(step_id)`
- `context.write_foreach_checkpoint(step_id, item_index, item_input, result)`
- `context.update_step_progress(step_id, payload)`
- `context._secret_values`
- `PipelineContext._refresh_credential(...)`
...
Runtime modules importing `engine.py` symbols:

- [src/brix/api.py](/root/docker/brix/src/brix/api.py): `PipelineEngine`
- [src/brix/cli.py](/root/docker/brix/src/brix/cli.py): `PipelineEngine`
- [src/brix/debug_tools.py](/root/docker/brix/src/brix/debug_tools.py): `PipelineEngine`, `_RenderedStep`
- [src/brix/mcp_handlers/_shared.py](/root/docker/brix/src/brix/mcp_handlers/_shared.py): `PipelineEngine`
- [src/brix/mcp_handlers/pipelines.py](/root/docker/brix/src/brix/mcp_handlers/pipelines.py): `LEGACY_ALIASES`
- [src/brix/mcp_handlers/runs.py](/root/docker/brix/src/brix/mcp_handlers/runs.py): `PipelineEngine`
- [src/brix/mcp_handlers/steps.py](/root/docker/brix/src/brix/mcp_handlers/steps.py): `LEGACY_ALIASES`
- [src/brix/mcp_server.py](/root/docker/brix/src/brix/mcp_server.py): `PipelineEngine`
- [src/brix/resilience.py](/root/docker/brix/src/brix/resilience.py): `_RenderedStep`
- [src/brix/testing.py](/root/docker/brix/src/brix/testing.py): `PipelineEngine`
- [src/brix/triggers/runners.py](/root/docker/brix/src/brix/triggers/runners.py): `PipelineEngine`
- [src/brix/validator.py](/root/docker/brix/src/brix/validator.py): `LEGACY_ALIASES`

Test modules importing `engine.py` symbols are listed in Section 7.

### Public API vs internal API

Likely intended public runtime API:

- `PipelineEngine`
- `LEGACY_ALIASES`

Internal-by-naming but externally consumed:
...
- `_measure_rss_mb`
- `_total_ram_mb`
- `_warn_if_high_memory`
- `_step_config_dict`

Implication: splitting this file is not just internal cleanup. Several underscore-prefixed symbols are imported by tests and some runtime modules. Either re-export them from `brix.engine` or update every downstream import.

## 7. Test Coverage Map

### Test files importing from `engine.py`

- [tests/debug_v3_pipeline.py](/root/docker/brix/tests/debug_v3_pipeline.py)
  Tests: ad hoc debug script using `PipelineEngine`; not formal coverage.

- [tests/test_advanced_flow.py](/root/docker/brix/tests/test_advanced_flow.py)
  Tests: advanced flow runners (`queue`, `emit`, debounce-related behavior). Engine relevance is mostly runner registration and integration.

- [tests/test_alerting.py](/root/docker/brix/tests/test_alerting.py)
  Tests: alerting subsystem; engine integration verifies runs produce alert-evaluable output.

- [tests/test_backward_compat.py](/root/docker/brix/tests/test_backward_compat.py)
  Tests: legacy CLI and v1 pipeline compatibility. Exercises `PipelineEngine.run()` through old pipeline formats and legacy flat step names.

- [tests/test_brick_composition.py](/root/docker/brix/tests/test_brick_composition.py)
  Tests: `_apply_profile`, `_resolve_runner` dynamic dispatch, and composition behavior.

- [tests/test_brick_first_engine.py](/root/docker/brix/tests/test_brick_first_engine.py)
  Tests: brick registry resolution, system brick mapping, `LEGACY_ALIASES`, and runner lookup.

- [tests/test_dag_execution.py](/root/docker/brix/tests/test_dag_execution.py)
  Tests: `_detect_dag_mode`, `_toposort_steps`, simple DAG scheduling, output visibility, diamond graph, cycle detection.

- [tests/test_dag_feature_parity.py](/root/docker/brix/tests/test_dag_feature_parity.py)
  Tests: explicitly covers DAG parity for `pause_before`, `test_mode`, pin mocks, and cache hits.

- [tests/test_db_only_handlers.py](/root/docker/brix/tests/test_db_only_handlers.py)
  Tests: DB-backed MCP handlers; one integration test verifies step-level credentials injection via engine.

- [tests/test_deprecation_enforcement.py](/root/docker/brix/tests/test_deprecation_enforcement.py)
  Tests: deprecated alias tracking, `RunResult.deprecation_warnings`, and `strict_bricks`.

- [tests/test_deps.py](/root/docker/brix/tests/test_deps.py)
  Tests: dependency management utilities and pipeline-level dependency install behavior through engine integration.

- [tests/test_engine.py](/root/docker/brix/tests/test_engine.py)
  Tests: core sequential engine behavior including ordering, `when`, `on_error`, output handling, and broad run behavior.

- [tests/test_engine_mcp_pool.py](/root/docker/brix/tests/test_engine_mcp_pool.py)
  Tests: `run(..., mcp_pool=...)` lifecycle semantics, ownership, cleanup, and wiring into `McpRunner`.

- [tests/test_engine_validate_config_allowlist.py](/root/docker/brix/tests/test_engine_validate_config_allowlist.py)
  Tests: `_VALIDATE_CONFIG_TOP_LEVEL_FIELDS` correctness and regression coverage for runner validation payload fields.

- [tests/test_flow_pipeline_config_bug.py](/root/docker/brix/tests/test_flow_pipeline_config_bug.py)
  Tests: flow.pipeline config promotion bug and foreach resolve errors being recorded instead of disappearing.

- [tests/test_helper_registry.py](/root/docker/brix/tests/test_helper_registry.py)
  Tests: helper registry plus engine integration for `step.helper -> step.script` resolution and failure when helper missing.

- [tests/test_idempotency.py](/root/docker/brix/tests/test_idempotency.py)
  Tests: run idempotency short-circuit and normal execution when no hit.

- [tests/test_imp_engine.py](/root/docker/brix/tests/test_imp_engine.py)
  Tests: brick default merging, validation payload precedence, and project auto-annotation into run notes.

- [tests/test_intra_step_progress.py](/root/docker/brix/tests/test_intra_step_progress.py)
  Tests: intra-step progress parsing and `_RenderedStep` integration.

- [tests/test_pin_mock.py](/root/docker/brix/tests/test_pin_mock.py)
  Tests: DB pin CRUD plus engine short-circuiting on pinned steps and test mode dry behavior.

- [tests/test_pipeline_group_runner.py](/root/docker/brix/tests/test_pipeline_group_runner.py)
  Tests: `flow.pipeline_group` runner integration through engine.

- [tests/test_pipeline_runner.py](/root/docker/brix/tests/test_pipeline_runner.py)
  Tests: `flow.pipeline` sub-pipeline runner integration through engine.

- [tests/test_poc_brick_pipeline.py](/root/docker/brix/tests/test_poc_brick_pipeline.py)
  Tests: end-to-end brick pipeline flow and alias resolution.

- [tests/test_progress.py](/root/docker/brix/tests/test_progress.py)
  Tests: `ProgressReporter` plus engine-driven foreach progress emission.

- [tests/test_progress_everywhere.py](/root/docker/brix/tests/test_progress_everywhere.py)
  Tests: runner-level progress calls across multiple runner types and engine integration points.

- [tests/test_python_jinja_rendering.py](/root/docker/brix/tests/test_python_jinja_rendering.py)
  Tests: `_RenderedStep` behavior as consumed by `PythonRunner`.

- [tests/test_repeat_runner.py](/root/docker/brix/tests/test_repeat_runner.py)
  Tests: repeat runner semantics and propagation of child outputs via engine state.

- [tests/test_resilience.py](/root/docker/brix/tests/test_resilience.py)
  Tests: circuit breaker, rate limiter, brick cache, saga pieces; engine integration verifies the resilience hooks fire.

- [tests/test_run_persistence.py](/root/docker/brix/tests/test_run_persistence.py)
  Tests: DB persistence tables and engine integration for `record_run_input` and step execution recording.

- [tests/test_secret_variables.py](/root/docker/brix/tests/test_secret_variables.py)
  Tests: `_redact_secret_values` and secret-variable DB behavior.

- [tests/test_self_healing.py](/root/docker/brix/tests/test_self_healing.py)
  Tests: `_ensure_step_requirements` and engine pre-execution dependency installation.

- [tests/test_smoke_regression.py](/root/docker/brix/tests/test_smoke_regression.py)
  Tests: broad end-to-end smoke coverage touching persistence, variables, store, secret redaction, brick resolution, and deprecation warnings.

- [tests/test_step_cache.py](/root/docker/brix/tests/test_step_cache.py)
  Tests: step-level cache manager and engine cache-hit short-circuit behavior.

- [tests/test_stop_step_skipped.py](/root/docker/brix/tests/test_stop_step_skipped.py)
  Tests: sequential `stop` step skip/execute semantics for multiple `when` forms.

- [tests/test_trigger_chaining.py](/root/docker/brix/tests/test_trigger_chaining.py)
  Tests: trigger chaining pipeline completion semantics, using `PipelineEngine.run` in trigger flows.

- [tests/test_triggers.py](/root/docker/brix/tests/test_triggers.py)
  Tests: trigger system state and pipeline-completion event generation through engine integration.

- [tests/test_v5_managed_helpers.py](/root/docker/brix/tests/test_v5_managed_helpers.py)
  Tests: dual-path helper resolution in engine and managed helper storage.

- [tests/test_v6_cancel_run.py](/root/docker/brix/tests/test_v6_cancel_run.py)
  Tests: cancellation sentinel behavior, cancel cleanup, and foreach cancel polling in engine.

- [tests/test_v6_llm_cost_tracking.py](/root/docker/brix/tests/test_v6_llm_cost_tracking.py)
  Tests: `_extract_step_cost`, sequential cost accumulation, and persistence of `cost_usd`.

- [tests/test_v6_retry_profiles.py](/root/docker/brix/tests/test_v6_retry_profiles.py)
  Tests: retry profiles, retry config fallback, backoff behavior, and engine retry integration.

- [tests/test_v7_04_execution_data.py](/root/docker/brix/tests/test_v7_04_execution_data.py)
  Tests: `persist_output`, `_should_persist`, `_context_snapshot`, and execution-data integration.

- [tests/test_v7_05_run_diff_mcp_trace_env.py](/root/docker/brix/tests/test_v7_05_run_diff_mcp_trace_env.py)
  Tests: `_capture_environment`, `_persist_step_output` storing `_mcp_trace`, run-diff environment support.

- [tests/test_v7_06_debug_tools.py](/root/docker/brix/tests/test_v7_06_debug_tools.py)
  Tests: `_write_context_snapshot`, `_wait_for_breakpoint_resume`, pause-before integration, inspect-context tooling.

- [tests/test_v7_07_resource_monitoring.py](/root/docker/brix/tests/test_v7_07_resource_monitoring.py)
  Tests: `_measure_rss_mb`, `_total_ram_mb`, `_warn_if_high_memory`, resource usage in statuses, container ID run metadata, timeline support.

- [tests/test_v8_compositor_mode.py](/root/docker/brix/tests/test_v8_compositor_mode.py)
  Tests: compositor-mode model behavior and engine runtime enforcement.

- [tests/test_v8_specialist.py](/root/docker/brix/tests/test_v8_specialist.py)
  Tests: specialist runner registration inside engine and extraction semantics.

### Blocks with weak or no direct test coverage

These are the highest-risk uncovered or lightly covered areas visible from the test map:
...
- Entire parity gaps with little or no explicit tests:
  - DAG omission of `validate_config`
  - DAG omission of `context-snapshot.json`
  - DAG omission of `step_outputs` persistence
  - DAG omission of `step_executions` persistence
  - DAG omission of resource usage attachment
  - DAG omission of LLM cost accumulation
  - DAG omission of saga compensation
  - DAG omission of cancellation polling outside breakpoints
  - DAG inability to execute `foreach`
...
- What could break:
  - `PythonRunner`, `debug_tools`, and `resilience` import `_RenderedStep` directly.
  - Any subtle field-copy behavior in `_RenderedStep.__init__` is relied on by multiple runners.
- Tests likely affected:
  - `tests/test_python_jinja_rendering.py`
  - `tests/test_intra_step_progress.py`
  - `tests/test_pipeline_runner.py`
  - `tests/test_repeat_runner.py`
- Circular import risk:
  - High if `_RenderedStep` moves into a module that imports runner classes or `PipelineEngine`.
  - Low if moved to a pure `engine_types.py` importing only `Step` and `PipelineLoader`.

### Move 2: Extract runner resolution/profile/default-merging logic

- What could break:
  - `strict_bricks` enforcement and deprecation warning accumulation are engine-stateful.
  - `LEGACY_ALIASES` is imported by validators and handlers.
- Tests likely affected:
  - `tests/test_brick_first_engine.py`
  - `tests/test_brick_composition.py`
  - `tests/test_deprecation_enforcement.py`
  - `tests/test_imp_engine.py`
- Circular import risk:
  - Medium if extracted code imports `PipelineEngine` or DB-layer code from places that already import `engine`.

### Move 3: Extract sequential step executor

- What could break:
  - Shared mutation of `step_statuses`, `last_output`, `pipeline_aborted`, `total_cost_usd`, and saga tracker.
  - The exact ordering between status writes, context writes, progress calls, and DB persistence.
- Tests likely affected:
  - `tests/test_engine.py`
  - `tests/test_flow_pipeline_config_bug.py`
  - `tests/test_pin_mock.py`
  - `tests/test_step_cache.py`
  - `tests/test_v7_04_execution_data.py`
  - `tests/test_v7_05_run_diff_mcp_trace_env.py`
  - `tests/test_v7_07_resource_monitoring.py`
  - `tests/test_v6_llm_cost_tracking.py`
- Circular import risk:
  - Medium if the extracted executor imports `PipelineEngine` instead of receiving an engine façade/protocol.

### Move 4: Extract foreach executor

- What could break:
  - Resume checkpoint semantics.
  - Shared `self._run_db` usage.
  - Progress updates and metadata snapshots used by `get_run_status`.
- Tests likely affected:
  - `tests/test_flow_pipeline_config_bug.py`
  - `tests/test_progress.py`
  - `tests/test_v6_cancel_run.py`
  - `tests/test_engine.py` where foreach behavior is indirectly covered
- Circular import risk:
  - Low if foreach helpers live under a shared executor module and receive dependencies as parameters.

### Move 5: Extract DAG scheduler into `engine_dag.py`

- What could break:
  - Nonlocal/shared mutation of `pipeline_aborted`, `last_output`, `stop_step_success`, `step_statuses`.
  - Event signaling correctness; a missed `done_events[step.id].set()` can deadlock the whole DAG.
  - Existing parity gaps could be preserved accidentally or changed unintentionally.
- Tests likely affected:
  - `tests/test_dag_execution.py`
  - `tests/test_dag_feature_parity.py`
  - `tests/test_v8_compositor_mode.py`
- Circular import risk:
  - Medium if `engine_dag.py` imports the full engine while engine imports the DAG executor.
  - Prefer inversion: `engine.py` imports a thin `run_dag(engine, ...)` function or a class that depends on a protocol, not the reverse.

### Move 6: Extract run finalization/history persistence

- What could break:
  - `RunResult` shape, deprecation warnings, cancellation handling, `history.record_finish`, trigger completion state, alert invocation.
  - The exact `all_ok` calculation and stop-step override.
- Tests likely affected:
  - `tests/test_run_persistence.py`
  - `tests/test_v6_llm_cost_tracking.py`
  - `tests/test_trigger_chaining.py`
  - `tests/test_triggers.py`
  - `tests/test_alerting.py`
  - `tests/test_smoke_regression.py`
- Circular import risk:
  - Low to medium. Finalization code imports history, triggers, and alerting lazily already, which helps.

### Move 7: Extract debug/persistence helpers

- What could break:
  - `persist_output`, `_mcp_trace` merge behavior, context snapshot shape, live debug tools.
- Tests likely affected:
  - `tests/test_v7_04_execution_data.py`
  - `tests/test_v7_05_run_diff_mcp_trace_env.py`
  - `tests/test_v7_06_debug_tools.py`
- Circular import risk:
  - Low if helpers remain pure and do not import `PipelineEngine`.
...
- `LEGACY_ALIASES`
- `_VALIDATE_CONFIG_TOP_LEVEL_FIELDS`
- public façade `PipelineEngine`
- module logger helpers if desired for backward compatibility re-export
- re-exports for moved underscore symbols still imported elsewhere

Exact method list to keep in `PipelineEngine`:
...
- `_apply_profile`
- `_apply_brick_defaults`
- `_resolve_runner`
- `_resolve_step_credentials`
- `_step_credentials_context`
- `_should_persist`
- `_context_snapshot`
- `_capture_environment`
- `_persist_step_output`
- `_write_context_snapshot`
- `_wait_for_breakpoint_resume`
- `_ensure_step_requirements`
- `_execute_with_retry`
- `_chunk_items`
- `_is_run_cancelled`
- `_run_foreach_sequential`
- `_run_foreach_parallel`
- `_detect_dag_mode`
- `_toposort_steps`
- `_run_dag`
- `_build_foreach_result`
...
- `EngineRuntimeView` protocol or just pass the actual engine instance
  Required attributes: `loader`, `progress`, `_runners`, `_brick_registry`, `_strict_bricks`, `_current_pipeline_name`, `_deprecation_db`, `_deprecation_warnings`, `_run_db`

### `engine_sequential.py`

Purpose: sequential orchestration and foreach logic.
...
Purpose: DAG orchestration only.
...
Important design note: `engine_dag.py` should call shared functions from `engine_step.py` instead of duplicating logic. That is where parity gaps should be fixed.
...
- What to move:
  - Nothing yet. Create tests or assertions around existing imports.
- What to update:
  - Add or confirm re-export expectations in `brix.engine` for `PipelineEngine`, `LEGACY_ALIASES`, `_RenderedStep`, `_VALIDATE_CONFIG_TOP_LEVEL_FIELDS`, `_extract_step_cost`, `_redact_secret_values`.
- How to verify:
  - Run `tests/test_brick_first_engine.py`
  - Run `tests/test_engine_validate_config_allowlist.py`
  - Run `tests/test_v6_llm_cost_tracking.py`
  - Run `tests/test_secret_variables.py`
- Rollback:
  - No structural move yet; revert the new compatibility assertions only.

### 2. Extract pure helpers into `engine_step.py`
...
- What to move:
  ...
  - `_RenderedStep`
- What to update:
  - `engine.py` re-imports and re-exports these symbols.
  - Keep old names alive from `brix.engine`.
- How to verify:
  - Run `tests/test_python_jinja_rendering.py`
  - Run `tests/test_intra_step_progress.py`
  - Run `tests/test_secret_variables.py`
  - Run `tests/test_v6_llm_cost_tracking.py`
- Rollback:
  - Restore definitions into `engine.py` and remove re-export imports.

### 3. Extract shared debug/persistence helpers
...
- What to move:
  ...
  - `_wait_for_breakpoint_resume`
- What to update:
  - `PipelineEngine` keeps delegating wrapper methods so tests and callers do not notice the move.
- How to verify:
  - Run `tests/test_v7_04_execution_data.py`
  - Run `tests/test_v7_05_run_diff_mcp_trace_env.py`
  - Run `tests/test_v7_06_debug_tools.py`
  - Run `tests/test_v7_07_resource_monitoring.py`
- Rollback:
  - Move helper bodies back into `PipelineEngine`.

### 4. Extract runner/profile/credential resolution primitives
...
- What to move:
  ...
  - `_step_credentials_context`
- What to update:
  - Keep `PipelineEngine` methods delegating to shared functions.
  - Ensure `LEGACY_ALIASES` remains imported from `brix.engine`.
- How to verify:
  - Run `tests/test_brick_composition.py`
  - Run `tests/test_brick_first_engine.py`
  - Run `tests/test_deprecation_enforcement.py`
  - Run `tests/test_imp_engine.py`
- Rollback:
  - Inline these methods back into `PipelineEngine`.

### 5. Extract dependency/cancellation/retry primitives
...
- What to move:
  ...
  - `_is_run_cancelled`
- What to update:
  - Delegating methods on `PipelineEngine`
  - Shared executor modules call these helpers rather than local copies
- How to verify:
  - Run `tests/test_self_healing.py`
  - Run `tests/test_v6_retry_profiles.py`
  - Run `tests/test_v6_cancel_run.py`
- Rollback:
  - Re-inline helper implementations into `PipelineEngine`.

### 6. Extract foreach logic into `engine_sequential.py`
...
- What to move:
  ...
  - `_build_foreach_result`
- What to update:
  - `PipelineEngine` wrappers delegate to extracted implementation
  - Sequential executor uses extracted functions
- How to verify:
  - Run `tests/test_flow_pipeline_config_bug.py`
  - Run `tests/test_progress.py`
  - Run `tests/test_v6_cancel_run.py`
  - Run `tests/test_engine.py`
- Rollback:
  - Restore foreach helpers inside `PipelineEngine`.

### 7. Extract sequential loop orchestration

- What to move:
  - The body of the non-DAG `for step in pipeline.steps` loop plus its direct supporting state into `run_sequential(...)`
- What to update:
  - `PipelineEngine.run()` becomes “preflight -> choose path -> finalize”
  - Introduce a small `SequentialRunState` return object instead of sharing many locals
- How to verify:
  - Run `tests/test_engine.py`
  - Run `tests/test_pin_mock.py`
  - Run `tests/test_step_cache.py`
  - Run `tests/test_run_persistence.py`
  - Run `tests/test_v7_07_resource_monitoring.py`
  - Run `tests/test_smoke_regression.py`
- Rollback:
  - Paste the orchestration body back into `run()`.

### 8. Extract DAG scheduler into `engine_dag.py` without changing behavior yet
...
- What to move:
  ...
  - `_run_dag`
- What to update:
  - `PipelineEngine.run()` calls extracted `run_dag(...)`
  - `PipelineEngine` keeps wrapper methods for compatibility
- How to verify:
  - Run `tests/test_dag_execution.py`
  - Run `tests/test_dag_feature_parity.py`
  - Run `tests/test_v8_compositor_mode.py`
- Rollback:
  - Move DAG logic back into `engine.py`.

### 9. Close the existing DAG parity gaps before further cleanup

- What to move:
  - No new move. This is behavior alignment work inside `engine_dag.py`.
- What to update:
  - Add shared validation call, context snapshot writing, progress persistence, resource measurement, step-output persistence, step-execution persistence, cost tracking, cancellation polling, and saga compensation semantics to DAG.
  - Decide whether `foreach` in DAG is forbidden explicitly or supported fully.
- How to verify:
  - Existing DAG tests
  - New tests covering each parity gap
  - Regression runs of `tests/test_v7_04_execution_data.py`, `tests/test_v7_05_run_diff_mcp_trace_env.py`, `tests/test_v7_07_resource_monitoring.py`, `tests/test_v6_llm_cost_tracking.py`, `tests/test_resilience.py`
- Rollback:
  - Revert only the parity changes while keeping the module split if needed.

### 10. Extract run finalization into a dedicated helper or module

- What to move:
  - The `finally` block logic that computes `final_result`, `all_ok`, cleanup, `history.record_finish`, cancellation handling, trigger-state updates, and alerting.
- What to update:
  - `PipelineEngine.run()` should pass a compact `RunArtifacts` structure into `finalize_run(...)`
- How to verify:
  - Run `tests/test_run_persistence.py`
  - Run `tests/test_alerting.py`
  - Run `tests/test_trigger_chaining.py`
  - Run `tests/test_triggers.py`
  - Run `tests/test_smoke_regression.py`
- Rollback:
  - Restore finalization block inline in `run()`.

### 11. Remove dead duplication and keep compatibility wrappers

- What to move:
  - Replace remaining duplicated logic between sequential and DAG with shared primitives from `engine_step.py`
- What to update:
  - Keep wrapper methods on `PipelineEngine` for one release cycle if this is a library/API used elsewhere
- How to verify:
  - Full engine-related test subset listed in this document
- Rollback:
  - Re-enable wrappers that directly contain old logic if necessary.
...
- What to update:
  - Ensure import paths in runtime modules and tests still use `brix.engine` unless intentionally migrated.
- How to verify:
  - Minimum high-signal suite:
    ...
    - `tests/test_v6_retry_profiles.py`
    - `tests/test_v7_04_execution_data.py`
    - `tests/test_v7_05_run_diff_mcp_trace_env.py`
    - `tests/test_v7_06_debug_tools.py`
    - `tests/test_v7_07_resource_monitoring.py`
    ...
    - `tests/test_smoke_regression.py`
- Rollback:
  - Revert the refactor branch to the last green commit if any of the path-parity tests regress.

## Summary Refactoring Judgment

`engine.py` is currently three things at once:

- the public execution façade,
- the shared step-execution policy layer,
- and two partially duplicated schedulers.

The safest extraction order is:

1. move pure/shared helpers first,
2. keep `PipelineEngine` as a delegating façade,
3. split sequential and DAG orchestration second,
4. then explicitly close DAG parity gaps rather than preserving them by accident.

The single biggest technical risk is not the file split itself. It is the fact that the DAG path is behaviorally incomplete relative to the sequential path. If the refactor simply relocates code without first making those gaps visible, the codebase will remain easier to read but still semantically inconsistent.
