...
def test_all_builtins_count():
    assert len(ALL_BUILTINS) >= 10  # started at 10, grew over time
...
def test_builtin_names():
    """Verify all original 10 expected brick names are present (superset OK)."""
    expected = {
    ...
    assert expected.issubset(actual), f"Missing bricks: {expected - actual}"
...
def test_builtin_types():
    ...
    # Original types
    valid_types = {
        "http", "cli", "python", "mcp", "filter", "transform", "file", "pipeline", "specialist",
        # Added by T-BRIX-DB-05c: system brick types (one per runner)
        "set", "choose", "parallel", "repeat", "notify", "approval", "validate",
        "db_query", "db_upsert", "llm_batch", "markitdown", "source",
        "switch", "merge", "error_handler", "wait", "dedup", "aggregate",
        "flatten", "diff", "respond", "pipeline_group",
    }
    for brick in ALL_BUILTINS:
