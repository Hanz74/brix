...
def test_workdir_base_is_persistent():
    """WORKDIR_BASE must be under ~/.brix/ (persistent volume), not /tmp/."""
    assert str(WORKDIR_BASE).startswith(str(Path.home() / ".brix")), (
        f"WORKDIR_BASE must be under ~/.brix/, got: {WORKDIR_BASE}"
    )
    assert "/tmp" not in str(WORKDIR_BASE), (
        f"WORKDIR_BASE must not be under /tmp (not persistent), got: {WORKDIR_BASE}"
    )
