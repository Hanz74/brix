...
# ---------------------------------------------------------------------------
# McpRunner — pool path timeout propagation (INBOX-264)
# ---------------------------------------------------------------------------


async def test_mcp_runner_pool_path_passes_timeout(tmp_path: Path):
    """McpRunner passes the step timeout to pool.call_tool (INBOX-264).

    Ensures that when a pool is attached, the per-step timeout is forwarded
    so pool.call_tool can apply it to session.call_tool and prevent hangs.
    """
    from unittest.mock import AsyncMock

    mock_pool = AsyncMock()
    mock_pool.call_tool = AsyncMock(
        return_value={"success": True, "data": {"ok": True}, "duration": 0.1}
    )

    runner = McpRunner(pool=mock_pool)
    step = _Step(server="fake", tool="my_tool", params={"x": 1}, timeout="45s")

    result = await runner.execute(step, context=None)

    assert result["success"] is True
    mock_pool.call_tool.assert_called_once_with(
        "fake", "my_tool", {"x": 1}, timeout=45.0
    )
...
async def test_mcp_runner_pool_path_uses_default_timeout(tmp_path: Path):
    ...
    mock_pool = AsyncMock()
    mock_pool.call_tool = AsyncMock(
        return_value={"success": True, "data": {}, "duration": 0.05}
    )

    runner = McpRunner(pool=mock_pool)
    step = _Step(server="fake", tool="my_tool", params={})  # no timeout attr
