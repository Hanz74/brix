...
def _normalize_step_config(step: dict) -> dict:
    """Normalize a step dict without rewriting persisted config placement.

    MCP callers may provide both runtime ``params`` and brick ``config``.
    Those must remain distinct so DB-backed steps persist runtime params in
    ``params_json`` and brick configuration in ``config_json``.
