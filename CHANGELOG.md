...
### Fixed
- ENGINE_ERROR 'cancel scope in different task' when running pipelines via MCP async dispatch
- `PipelineEngine.run()` now uses `async with McpConnectionPool()` instead of manual `__aenter__`/`__aexit__` calls, ensuring the `ClientSessionGroup` cancel scope is always entered and exited in the same asyncio Task — even when `engine.run()` is dispatched via `asyncio.create_task()` (async mode in MCP server)

## [3.0.0] — 2026-03-22

### Added (T-BRIX-V3-01)
- MCP Connection Pooling: Sessions cached per pipeline run via `McpConnectionPool`
- `PipelineEngine` creates a pool at the start of `run()` and closes it in the `finally` block
- `McpRunner` accepts an optional `pool` parameter and property; uses the pool when available, falls back to per-call stdio connections for backward compatibility
- Pool is attached to `McpRunner` at run start and cleared after run (even on error)
- 7 new tests across `tests/test_engine.py` and `tests/test_mcp_pool.py` covering pool lifecycle, pool/runner integration, reuse across calls, and backward-compat fallback
